...
class SftpSync(object):
    ...
    def __init__(self):
        ...
        try:
            self.pkey = paramiko.RSAKey.from_private_key_file(
                os.path.expandvars(vim.eval("g:sftpsync_private_key_file"))
            )
        except FileNotFoundError:
            self.pkey = None
    ...
    def upload(self, filename, target):
        ...
        for (project_name, project) in vim.eval("g:sftpsync_projects").items():
            try:
                (destination, count) = re.subn(
                    project["source"], project["destination"][target]["directory"], filename, flags=re.IGNORECASE
                )
                ...
                for host in config["hosts"]:
                    ...
                    try:
                        ...
                        if cache_key in self.connections and self.connections[cache_key][0].get_transport().is_active():
                            ...
                            if not self.pkey and not password:
                                raise FileNotFoundError(
                                    "Neither private key file nor password is set, please adjust configuration"
                                )
                        ...
                        try:
                            ...
                            path = "/".join(
                                destination_dir.split("/")[
                                    0 : len(project["destination"][target]["directory"].split("/"))
                                ]
                            )
                            subdirs = destination_dir.split("/")[
                                len(project["destination"][target]["directory"].split("/")) : -1
                            ]
